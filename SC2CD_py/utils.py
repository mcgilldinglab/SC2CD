...
def clustering(adata,n_clusters, cell_dec=None,w=0, radius=50, key='emb', method='mclust', start=0.1, end=3.0, increment=0.01, refinement=False,new_label='domain'):
    """\
...
def calculate_neigh_purity_score(adata, n_cluster, n_neigh=8):
    n_neigh = 8
    domain = adata.obs['domain'].astype(int).values
    ...
    for i in range(n_cell):
        ...
        for j in range(1, n_neigh+1):
            ...
            if n_cl == curr_cl:
                same_neigh += 1
        purity[curr_cl-1] += same_neigh/n_neigh
 
    unique_elements, counts = np.unique(domain, return_counts=True)
    purity[unique_elements-1] = purity[unique_elements-1]/counts
    mean_neigh_score = purity.sum()/n_cluster
    
    return purity, mean_neigh_score
